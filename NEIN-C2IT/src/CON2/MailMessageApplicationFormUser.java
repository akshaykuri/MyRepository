...
//@WebServlet("/MailMessageApplicationFormUser")
public class MailMessageApplicationFormUser {
	String driver;
	Date gdate = null;

//String form2_no="NEIN/CS/APP-FORM/90-17";

public String getMailMessageAppForm(String form)
{
	//form=form2_no;
	StringBuffer stringBuffer = new StringBuffer();
	Connection con=null;
	PreparedStatement pst=null,pst2=null,pst3=null,pst4=null,pst5=null,pst44=null;   
	ResultSet resultSet=null,rs2=null,rs3=null,rs4=null,rs5=null,rs44=null;
	try{
...
pst5 = con.prepareStatement("select * from neinapplicationform nein left outer join branch_master_main br on br.m_b_no=nein.requested_branch where applicationNo=?");
	pst5.setString(1, form);
	rs5 = pst5.executeQuery();
	while(rs5.next()){
	branch = rs5.getString("branch");
	depart = rs5.getString("department");
	full_name = rs5.getString("full_name");
	emp_code = rs5.getString("employee_code");
	designation = rs5.getString("designation");
	r_officer = rs5.getString("report_officer");
	scopeOfWork = rs5.getString("scope_Of_Work");
	requestBy = rs5.getString("requested_By");
	emp_type = rs5.getString("emp_type");
	emp_contract_type = rs5.getString("emp_contract_type");
	requestOption = rs5.getString("request_type");
	requested_branch = rs5.getString("m_b_name");
	requestFor = rs5.getString("requestFor");
	}
	
String domain="",empType="",mailRequest="",compProvMobile="",makeModel="",mobIntune="",mobNo="",imei1="",imei2="",emRemarks="",domRemarks="",bioRemarks="",folderRemarks="",internetReqType="",internetRemarks="",newinBranch="",newinOP="",newinReq="",newinRem="",nexComCode="",nexCostName="",nexCostCode="",nexOPRange="",nexReq="",nexOptiMenu="",nexRem="",gsnetBC="",gsnetdiv="",gsnetpri="",gsnetuser="",gsnetRem="",internalApplication="",internalRemarks="",usbAccessFor="",usbDetails="",usbRemarks="",vpnDomainID="",vpnEmailID="",vpnMPLS="",hardPenRemarks="",otherAssetRemarks="",compProvSimNumber="",compProvSimCard="",passReset="",chanReqBio="",selBioBranches="",glowProject="",glowWarehouse="",glowRemarks="",previousBranchesNewins="",changesNewins="",changesGSNet="",prevBRCodesGSNet="",prevDivGSNet="",prevPDivGSNet="",prevRoleGSNet="",newinsLevel="",newinAllowed="",newinBlocked="";
pst2 = con.prepareStatement("select emailDomain,group_name,employeeType,mailIDCreated,reqMailID,intuneAccessProvided,CompanyProvMobile,makeAndModel,mobIntune,mobileNo,imei1,imei2,emailRemarks,domainRemarks,bioRemarks,folderRemarks,internetReqType,internetRemarks,newinsBranchCode,newinsOpCode,newinsRequest,newinsRemarks,nexasCompCode,nexasCCName,nexasCCCode,nexasOPRange,case when nexasReqType='HOAcUser' then 'HO Acounting User' when nexasReqType='BRAcUser' then 'Branch Acounting User' when nexasReqType='ITUser' then 'IT User' when nexasReqType='RepoDispOnly' then 'Report display only User' end as nexasReqType ,nexasOptionalMenu,nexasRemarks,gsnetBranchCode,gsnetDivName,gsnetPrimaryDiv,gsnetUserRole,gsnetRemarks,internalApplication,internalRemarks,usbAccessFor,usbDetails,usbRemarks,vpnDomainID,vpnEmailID,vpnMPLS,hardPenRemarks,otherAssetRemarks,compProvSimCard,compProvSimNumber,passReset,chanReqBio,selBioBranches,glowProject,glowWarehouse,glowRemarks,previousBranchesNewins,changesNewins,changesGSNet,prevBRCodesGSNet,prevDivGSNet,prevPDivGSNet,prevRoleGSNet,newinsLevel,newinBlocked,newinAllowed from neinapplicationform_requestfor where app_no=?");
...
pst4 = con.prepareStatement("select m_b_name from branch_master_main where m_b_no in ("+selBioBranches+") ORDER BY m_b_name Asc");
rs4 = pst4.executeQuery();
while(rs4.next()){
	selBioBranches1 += rs4.getString(1)+", ";
}
}

String lvl="";
pst44 = con.prepareStatement("select levelno from neinapplicationform_item where form2_no='"+form+"' order by item_id desc limit 1");
rs44 = pst44.executeQuery();
while(rs44.next()){
	lvl = rs44.getString("levelno");
}
...
if(!selBioBranches1.equals("")){
	...
	if(lvl.equalsIgnoreCase("Creater")){
		stringBuffer.append("<tr>"+
		"<td style=\"width:25%;\" colspan=\"1\">Newins Level </td>"+
		"<td style=\"width:75%;\" colspan=\"3\" bgcolor=\"white\">"+newinsLevel+"</td> "+
		"</tr>");
		stringBuffer.append("<tr>"+
		"<td style=\"width:25%;\" colspan=\"1\">Newins Programs Allowed </td>"+
		"<td style=\"width:75%;\" colspan=\"3\" bgcolor=\"white\">"+newinAllowed+"</td> "+
		"</tr>");
		stringBuffer.append("<tr>"+
		"<td style=\"width:25%;\" colspan=\"1\">Newins Programs Blocked </td>"+
