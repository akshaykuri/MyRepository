  ...
  if (document.getElementsByName("mmdradio")[0].checked)
	 {
	  if (document.getElementsByName("higherAuthority")[1].checked) 
		     ...
		     if(ext=='')
	            {
	            alert("Please Uploade CCD Report File"+ext);
	            fi.focus();
	            return false;
	            } 
	  		}
...
function OtherAdmin()
    ...
    document.form.action="approvalRequestCCDBack.jsp";
...
Statement st5 = null,st11=null,st0=null,stnext=null; 
ResultSet rs5 = null,rs11=null,rsO=null,rsnext=null; 
try{
%>
...
<table width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  rs1 = st1.executeQuery("select form_no,desg,emp,status,remarks,form_date,mail,levelno from form13_ccdreport_item WHERE form_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
...
<%String flag = c2ITUtil1.getAuthorizedNameCCDREport(form2_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("EmpNo").toString()); %>
...
//rs5 = st5.executeQuery("Select * from desg");emp_no,levelno
rsl1 = stl1.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+req_Branch+"' and empid='"+session.getAttribute("EmpNo").toString()+"' and formtype='CCDReport'");
if(rsl1.next())
    { 
...
//System.out.println("Current allopt :"+req_Branch);
//System.out.println("Current dirappro :"+dirappro);
...
if(session.getAttribute("EmpNo").equals(nextemp))
    {
	  nextlevel++;
    }
  ...
  <tr style="visibility: hidden;display: none;" id="idApproved" >
   <td>
       <%if(!levelno.equals(maxlevel))
    	      ...
    	      if(dirappro.equals("1") && !levelno.equals("0"))
                     {%>
   ...
   <% 
   System.out.println("Next  Level no 2 :"+allopt);
     if(allopt.equals("0") ||allopt.equals("") )
    	 ...
    	 int next=0;
    	  while(curlev<=Integer.parseInt(maxlevel))
    	     {
    		  next=curlev+1;
    	     rs5 = st5.executeQuery("SELECT * FROM formapproval WHERE branch='"+req_Branch+"' and levelno='"+next+"' and formtype='CCDReport'  ORDER BY levelno ASC");
         	 if(rs5.next())
                 {
		         ResultSet nrs = nstmt.executeQuery("SELECT DISTINCT u.user_name,rr.role_code,b.branch_name, d.department_name, m3.user_name AS manager_name,u.email,u.mobile_number FROM user u Inner Join user_role ur on ur.user_id=u.user_id INNER JOIN reporting_officers m1 ON m1.user_id = u.user_id INNER JOIN role rr ON ur.role_id =rr.role_id INNER JOIN branchmaster b ON b.branch_id = u.branch_id INNER JOIN department d ON d.department_id = u.department_id INNER JOIN user m3 ON m3.user_id = m1.manager_id INNER JOIN user m4 ON m4.user_id = m1.manager2_id WHERE ( m1.emp_id ='" + rs5.getString(5) + "' ) AND u.employee_status = 'yes' ");
		         if(nrs.next())
