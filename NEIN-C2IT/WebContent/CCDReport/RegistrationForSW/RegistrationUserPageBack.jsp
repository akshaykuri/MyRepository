...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form14_ccdregistration WHERE form10_no IN(SELECT form10_no FROM form14_ccdregistration LEFT JOIN branch_master b ON(substr(form10_no,13,2)=b.b_no) WHERE b.b_no=? GROUP BY form10_no)");
...
//40
psmt = cn2.prepareStatement("INSERT INTO form14_ccdregistration(form10_no,f_date,idextend,branch,depart,full_name,designation,regis_mail,emp_code,Software,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,form_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt10 =cn2.prepareStatement("UPDATE form14_ccdregistration SET Approval_status=?,Approval_sttatuss=?,Approval_remarks=?,Approval_f_date=?,Approval_emp=?,Approval_desg=?,Approval_empp=?,Approval_desgg=? WHERE form10_no=?");
...
psmt2 = cn2.prepareStatement("INSERT INTO form14_ccdregistration_item(form10_no,desg,emp,status,remarks,f_date,mail,presence) VALUES(?,?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form14_ccdregistration_item(form10_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageCCDRegistration(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail,"NEIN-C2IT CCD User Registration FORM :  " +new_id+ " ",i_name+  " "+  "request your approval for the CCD user - Registration FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  http://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - http://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);              
message = null;




msg = "REGISTRATION__OF__USER__SAVED__AND__SENT__SUCCESSFULLY";
