...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form5_folder WHERE form5_no IN(SELECT form5_no FROM form5_folder LEFT JOIN branch_master b ON(substr(form5_no,17,2)=b.b_no) WHERE b.b_no=? GROUP BY form5_no)");
...
if(rs_id.next()){
	...
	try
	{
	...
	catch(SQLException ex)
	    {
		 ...
		 new_id = "NEIN/CS/Sfolder/"+i_b_no+"-1";
	    }
...
//40
psmt = cn2.prepareStatement("INSERT INTO form5_folder(form5_no,f_date,idextend,branch,depart,full_name,designation,scopeOfWork,emp_code,reporting_officer,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,form_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt.setString(23,"Server Folder Access Request");
...
psmt2 = cn2.prepareStatement("INSERT INTO form5_folder_item(form5_no,desg,emp,status,remarks,f_date,mail,empid,levelno,presence) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form5_folder_item(form5_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageFolder(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail," SHARED FOLDER FORM :  " +new_id+ " ",i_name+  " "+  "request you to approve - SHARED FOLDER FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://ninsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);              
...
response.sendRedirect("SharedFolderPage.jsp?msg=" +msg);
