 ...
 */
  ...
  <script language="javascript" type="text/javascript"> 
//function resendMail(mail,form5_no,email_id,name)
function resendMail(email_id,form5_no,mail,name)
  		 {
	if (typeof XMLHttpRequest != "undefined"){    
         xmlHttp= new XMLHttpRequest();    
           }      else if (window.ActiveXObject){
           alet("Creating html http object");    
             xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                    ...
                    alert("Browser does not support XMLHTTP Request");    
                      return;     
                       }       var url="../../ResendMailSharedFolder?mail="+email_id+","+form5_no+","+mail+","+name+"";  
...
<form  name="form" method="post"  action="SharedFolderAdminLocalBack.jsp">
...
<table width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  rs1 = st1.executeQuery("select form5_no,desg,emp,status,remarks,f_date,mail,empid,levelno from form5_folder_item WHERE form5_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=email_id%>','<%=form5_no%>','<%=m%>','<%=SessionName%>');" /></td> 
...
String flag =c2ITUtil1.getAuthorizedNameFolder(form5_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("EmpNo").toString()); %>
   ...
   <tr style="display: none;" id="idApproved" >
    ...
    cn5 = DriverManager.getConnection(url+dbName,userName,pass); 
     ...
     String maxlevel="";
	  ...
	  String i_b_no1=form5_no.substring(16,18);
	  
	  
	  
      rs12= st12.executeQuery("SELECT max(levelno) FROM formapproval WHERE branch='"+i_b_no1+"' and formtype='SHARRED_FOLDER'  ORDER BY levelno ASC");
     if(rs12.next())
         { 
    ...
    rs11 = st11.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+i_b_no1+"' and empid='"+emp_no+"' and formtype='SHARRED_FOLDER'");
     if(rs11.next())
         { 
     ...
     /* ////////////////////////// Skeeping level code  ////////////////////////////////// */
      ...
      rsnext = stnext.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='SHARRED_FOLDER' ORDER BY levelno ASC");
      if(rsnext.next())
           {
     ...
     if(allopt.equals("0") || allopt.equals(""))
    	  ...
    	  if(!levelno.equals(maxlevel) && !dirappro.equals("1"))
    	     ...
    	     rs5 = st5.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='SHARRED_FOLDER' ORDER BY levelno ASC");
    	     if(rs5.next())
                {
               ...
               %><input type="hidden" name="senddirect" id="senddirect" value="sendonly" readonly/>
               ...
               else
                  {
            	   ...
            	   if(!levelno.equals(maxlevel) && dirappro.equals("1"))
            	         ...
            	         <td style="display: none;" id="send">Send to HO(IT)<br>
                           ...
                           <td style="display: none;" id="direct">IT Dept. Use only<br>
...
<tr>
<td class="it2" style="text-align: right;width:23%;">IT  Remarks :</td>
