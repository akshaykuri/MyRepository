...
document.form.action="SharedFolderAdminBack.jsp";
document.form.method="post";
document.form.submit();
}
function ITManager()
{
document.form.action="SharedFolderAdminITmanagerBack.jsp";
document.form.method="post";
document.form.submit();
}
</script>
...
<table id="tableData" name="tableData" width="100%" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  rs1 = st1.executeQuery("select form5_no,desg,emp,status,remarks,f_date,mail from form5_folder_item WHERE form5_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=m%>','<%=form5_no%>','<%=empD%>','<%=SessionName%>');" /></td> 
...
<br />
	<b>Send To:</b>
		...
		cn11 = con.getConnection2();
		  ...
		  rs11 = st11.executeQuery("select form5_no,desg,emp,status,remarks,f_date,mail from form5_folder_item WHERE form5_no='"+req+"' ORDER BY id ASC"); 
		  while(rs11.next())
		  {
...
<%}
 ...
 <input type="hidden" value="<%= session.getAttribute("desg") %>" id="iddesg"/>
  <input type="hidden" value="<%= session.getAttribute("form5_no") %>" id="idmrId"/>
...
<!--  <table style="margin-left: auto;margin-right: auto;" ><tr>
     ...
     <td style="font-size:1">&nbsp;&nbsp;<input style="size:1;font-size: 1.1em;" type="button"  value="BACK" onclick="window.location.href='SharedFolderSelectAdmin.jsp?msg=1'"/> </td>
