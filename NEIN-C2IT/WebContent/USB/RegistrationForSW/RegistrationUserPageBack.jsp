...
String dongleNo= request.getParameter("dscNo");
...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form17_usb_access WHERE form10_no IN(SELECT form10_no FROM form17_usb_access LEFT JOIN branch_master b ON(substr(form10_no,10,2)=b.b_no) WHERE b.b_no=? GROUP BY form10_no)");
...
//40
psmt = cn2.prepareStatement("INSERT INTO form17_usb_access(form10_no,f_date,idextend,branch,depart,full_name,designation,regis_mail,emp_code,Software,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,form_name,accessFor,dongleNo) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt.setString(23,"USB Access Request");
psmt.setString(24,accessFor);
psmt.setString(25,dongleNo);
...
psmt10 =cn2.prepareStatement("UPDATE form17_usb_access SET Approval_status=?,Approval_sttatuss=?,Approval_remarks=?,Approval_f_date=?,Approval_emp=?,Approval_desg=?,Approval_empp=?,Approval_desgg=? WHERE form10_no=?");
...
psmt2 = cn2.prepareStatement("INSERT INTO form17_usb_access_item(form10_no,desg,emp,status,remarks,f_date,mail,presence) VALUES(?,?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form17_usb_access_item(form10_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageUSBAccess(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail,"NEIN-C2IT USB Access Request FORM :  " +new_id+ " ",i_name+  " "+  "request your approval for the USB ACCESS FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  http://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - http://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);              
message = null;




msg = "SENT__SUCCESSFULLY........";
