...
<head><title>USB ACCESS FORM</title>
...
<script type="text/javascript">
  ...
  {  var p1=document.form.branch1; var y1=document.form.year1;
   if ((p1.value==null)||(p1.value=="NULL"))
 				{alert("Select Registration Number!"); p1.focus(); return false; }
...
<tr align="center">
 <td bgcolor="#2f4377" class="blueLine"><strong><font size ="4" color="white">USB ACCESS FORM</font></strong></td>
</tr>
...
<form name="form" method="post" action="RegistrationLocalAllAdmin.jsp?msg=1">
<%-- %>	 <form action="POMonthWiseBack.jsp"> 
	<form action="POWiseBack.jsp">--%>
<%
	 String b_no=null,desg=null,DESG_NO=null;
	 
	 b_no = (String) session.getAttribute("b_no");
	 desg = (String) session.getAttribute("desg");
	 DESG_NO = (String) session.getAttribute("DESG_NO");
	 System.out.println("DESG_NO to Check All Software Users : "+DESG_NO);
	%>	 
...
if(b_no.equals("90"))
    rs9 = st9.executeQuery("SELECT DISTINCT form10_no,full_name,Software FROM  form17_usb_access WHERE form10_no LIKE 'NEIN/USB%' ORDER BY form10_no DESC");
else if(b_no.equals("30"))
	rs9 = st9.executeQuery("SELECT DISTINCT form10_no,full_name,Software FROM  form17_usb_access WHERE i_b_no in('30','32','11','53') and  form10_no LIKE 'NEIN/USB%' ORDER BY form10_no DESC");

	
else
	rs9 = st9.executeQuery("SELECT DISTINCT form10_no,full_name,Software FROM  form17_usb_access WHERE i_b_no='"+b_no+"' AND form10_no LIKE 'NEIN/USB%' ORDER BY idextend DESC");
...
<table align="center">
<tr>
<th>SELECT REGISTRATION FORM NUMBER</th>
</tr>
</table>
   ...
   <tr><!--<th>BRANCH:</th>
	--><td>
      ...
      <select  name="req"  id="hiderow">     <option value="NULL">SELECT</option> 
												...
												<option ><%=rs9.getString(1)%> - <%=rs9.getString(3)%> - <%=rs9.getString(2)%></option>
										        <%} %> 
										        </select>
      </td>
