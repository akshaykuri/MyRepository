...
int slno=0,sno=0;
String reqNo="",reqDate="",empID="",Name="",city="",desg="",dept="",email_id="",reqRem="",appName="",appDesg="",appStatus="",listOfApps="",appRemarks="",appDate="",verAppName="",verBy="",verRemarks="";
...
<%
	...
	rs3 = st3.executeQuery("select empName,empDesg,status,remarks,date_format(appDate,'%d-%m-%Y') as appDate from c2itreviewfromitem where reqNo in ('"+reqNo+"') and remarks<>'No Action Performed' order by item_id");
%>
...
<%
	st9  = cn1.createStatement();
	rs9  = st9.executeQuery("select branchName,branchNo from c2itreviewfrommainitems where reqNo='"+reqNo+"' order by itemsID");
	st10 = cn1.createStatement();
%>
...
<table width="100%" cellspacing="0" cellpadding="0" bgcolor="lightgrey" align="center">
	<tr>
		<td style="text-align: center;font-size: 20px;font-weight: bolder;width: 100%;" colspan="7">Verification Details</td>
	</tr>
	<tr style="background-color: black;font-size: 15px;color: white">
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Branch Name/Code</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Application Name</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Branch-IT Status</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Verifier Remarks</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Verification Status</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">Upload File</th>
		<th style="text-align: center;vertical-align: middle;" nowrap="nowrap">IT-Manager Remarks</th>
	</tr>
<%while(rs9.next()){
	bName  = rs9.getString("branchName")+" - "+rs9.getString("branchNo");
	brCode = rs9.getString("branchNo");
	brName = rs9.getString("branchName");
%>
				<tr>
					<td style="text-align: left;vertical-align: middle;width: 15%;font-weight:bold;font-style:italic;" rowspan="<%=snoLen%>"><%=bName%>
						<input type="text" value="<%=snoLen%>" name="sno" id="sno" style="display: none;"/>
						<input type="text" value="<%=brCode%>" name="brCode" id="brCode" style="display: none;"/>
						<input type="text" value="<%=brName%>" name="brName" id="brName" style="display: none;"/>
					</td>
<%
rs10 = st10.executeQuery("select distinct b.applicationName as an,case when a.verificationStatus='1' then 'Verified' else 'Not Verified' end as vs,a.verifiedBy as vb,date_format(a.verifiedOn,'%d-%m-%Y') as vo,b.verifierRemarks as vr from c2itreviewfrommainitems a left outer join c2itreviewfrom_verify b on a.reqNo=b.requestNo where a.reqNo='"+reqNo+"' and a.verificationStatus='1' and a.branchNo='"+brCode+"' and a.branchName='"+brName+"' group by b.v_id");
while(rs10.next()){
	ind++;
	verAppName = rs10.getString("an");
	verBy = rs10.getString("vs")+" by "+rs10.getString("vb")+" on "+rs10.getString("vo");
	verRemarks = rs10.getString("vr");
%>
					<td style="font-weight:bold;font-style:italic;text-align: left;vertical-align: middle;"><%=verAppName%></td>
<%if(ind == 1){%>
					<td style="font-weight:bold;text-align: left;vertical-align: middle;" rowspan="<%=snoLen%>"><%=verBy%></td>
<%}%>
					<td style="font-weight:bold;text-align: left;vertical-align: middle;"><%=verRemarks%></td>
					<td style="font-weight:bold;font-style:italic;text-align: center;vertical-align: middle;">
						<input type="radio" name="verStatus<%=sno%>" id="verStatus<%=sno%>" value="1" />&nbsp;&nbsp;Verified
					</td>
...
<%
	...
	DbUtil.closeDBResources(rs10,st10,cn1);
