...
ResultSet rscsout = stcsout.executeQuery("select employee_code from neinapplicationform where Approval_sttatuss in ('Pending') and full_name='"+s+"'");
while(rscsout.next()){
	flag=true;
}

ResultSet rscsout1 = stcsout.executeQuery("select emplCode from neinapplicationformmain where emplName='"+s+"' and (emplreqFor like '%Email ID%' and emplreqFor like '%Domain Account%' and emplreqFor like '%Biometric / Access Card%' and emplreqFor like '%Shared Folder%' and emplreqFor like '%Internet Access / FTP Access%' and emplreqFor like '%NEWINS%' and emplreqFor like '%NExAS%' and emplreqFor like '%GS-NET%' and emplreqFor like '%Internal Application%' and emplreqFor like '%USB Access%' and emplreqFor like '%VPN Access%' and emplreqFor like '%HardDisk / PenDrive%' and emplreqFor like '%Any Other Asset%' and emplreqFor like '%NEx-GLOW%')");
while(rscsout1.next()){
	flag=true;
}

if(flag!=true){
ResultSet rscsdb = stcsdb.executeQuery("select * from neinapplicationform nein left OUTER join neinapplicationform_item nein_item on nein.applicationNo=nein_item.form2_no left outer join neinapplicationform_requestfor nein_for on nein.applicationNo=nein_for.app_no where Approval_sttatuss in ('Closed') and full_name='"+s+"' and nein_item.presence='over' order by nein.form_id DESC LIMIT 0,1");
while(rscsdb.next()){
...
buffer = buffer + "<input type='text' id='r2' value='"+(rscsdb.getString("emp_type")==null?"":rscsdb.getString("emp_type"))+"'>";
...
buffer = buffer + "<input type='text' id='r58' value='"+rscsdb.getString("branch")+"'>";
buffer = buffer + "<input type='text' id='r59' value='"+rscsdb.getString("department")+"'>";
buffer = buffer + "<input type='text' id='r60' value='"+rscsdb.getString("designation")+"'>";
...
buffer = buffer + "<input type='text' id='r32' value='"+(rscsdb.getString("nexasOptionalMenu")==null?"":rscsdb.getString("nexasOptionalMenu"))+"'>";
