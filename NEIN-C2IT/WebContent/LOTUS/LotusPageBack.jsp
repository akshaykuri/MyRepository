...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form3_lotus  WHERE form3_no IN(SELECT form3_no FROM form3_lotus  LEFT JOIN branch_master b ON(substr(form3_no,15,2)=b.b_no) WHERE b.b_no=? GROUP BY form3_no)");
...
if(rs_id.next()){
	...
	try
	{
	...
	catch(SQLException ex)
	    {
		 ...
		 new_id = "NEIN/CS/Email/"+req_Branch+"-1";
	    }
...
//40
psmt = cn2.prepareStatement("INSERT INTO form3_lotus(form3_no,f_date,idextend,branch,depart,full_name,designation,scopeOfWork,emp_code,reporting_officer,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,New_email,password,form_name,emailDomain,employeeType,intune) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt.setString(23,New_email);
psmt.setString(24,password);
psmt.setString(25,"Email ID Request");
psmt.setString(26,emaildomain);
psmt.setString(27,emptype);
psmt.setString(28,intune);
...
psmt2.setString(10,"0");
int j = psmt2.executeUpdate();


psmt3 = cn2.prepareStatement("INSERT INTO form3_lotus_item(form3_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageLotus(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail," Email Request form :  " +new_id+ " ",i_name+  " "+  "request you to approve - E-mail FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
...
response.sendRedirect("LotusPage.jsp?msg=" +msg);
