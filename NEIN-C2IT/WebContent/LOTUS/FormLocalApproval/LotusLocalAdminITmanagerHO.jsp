...
psmt = cn2.prepareStatement("UPDATE form3_lotus_item f,(SELECT max(id) as idf FROM form3_lotus_item WHERE form3_no=? AND desg=? AND emp=?) as p SET f.status=?,f.remarks=?,f.f_date=?,f.presence=? WHERE f.form3_no=? AND f.desg=? AND f.emp=? AND f.id=p.idf ");
...
psmt2 = cn2.prepareStatement("UPDATE form3_lotus SET ITusername=?,ITpassword=?,ITcreated=?,ITremark=?,Approval_sttatuss=?,Approval_f_date=? WHERE form3_no=?");
