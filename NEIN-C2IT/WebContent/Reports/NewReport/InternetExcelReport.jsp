...
response.setHeader("Content-Disposition", "attachment; filename=InternetExcelReport.xls");%>
...
<head>
	...
	<title>BRANCH DATE TO DATE WISE INTERNET ACCESS</title>
...
<%!
	...
	%>
     ...
     req = request.getParameter("product5");
     req2 = request.getParameter("date5");
     req3 = request.getParameter("date55");
...
INTERNET ACCESS REPORT</font></b></center></td></tr>
</table>
				...
				<td  width="" bgcolor="lightgrey" align="center" style="vertical-align:middle" height="21" ><font size="2"><b>INITIATOR NAME</b></font></td>
				   ...
				   //refer :rs2 = st2.executeQuery("select          s.s_no,s.s_sExecutive,s.s_date,s.customer,s.s_person,s.s_contact,s.s_mail,s.s_add,s.operation_date,s.s_stage,s.s_add,s.s_web,CONCAT(s.s_value,' ',s.s_valueCurrency) AS Svalue,CONCAT(s.s_weight,' ',s.s_abbre) AS Sweight,CONCAT(s.s_profit,' ',s.s_profitCurrency) AS Sprofit,s.s_compet,m.m_m_date,m.m_m_stage,s.s_product,s.s_country,s.s_country2 FROM sales_master s INNER JOIN sales_meeting m ON (s.s_no=m.s_no) WHERE s.s_sExecutive = '"+req+"' and m.m_m_date >= '"+dateF+"' AND m.m_m_date<='"+dateFF+"' ORDER BY m.m_m_date ASC ");
				              if(req!=null && req.equals("all"))
				           {
				            ...
				            rs2 = st2.executeQuery("SELECT form_name,form6_no,i_name,i_desg,branch,Approval_empp,Approval_desgg,Approval_remarks,DATE_FORMAT(Approval_f_date,'%d/%m/%Y'),full_name,designation,reporting_officer,requestOption FROM form6_internet WHERE f_date >= '"+dateF+"' AND f_date<='"+dateFF+"' AND deleted='no' ORDER BY id ASC   ");
				           }
				           else
				           {
				        	   System.out.println("Specific BRANCH "+req);
				        	   rs2 = st2.executeQuery("SELECT form_name,form6_no,i_name,i_desg,branch,Approval_empp,Approval_desgg,Approval_remarks,DATE_FORMAT(Approval_f_date,'%d/%m/%Y'),full_name,designation,reporting_officer,requestOption FROM form6_internet WHERE branch='"+req+"' AND f_date >= '"+dateF+"' AND f_date<='"+dateFF+"' AND deleted='no' ORDER BY id ASC   ");
				           }
				          ...
				          // java.text.SimpleDateFormat sdf2 = new SimpleDateFormat("dd/mm/yyyy");
							...
							form_no = rs2.getString("form6_no");
					...
					<td width="" align="center"  height="21" style="vertical-align:middle"><font size="2"><%=initiatorDesg%></font></td>
						...
						<%
						rs3 = st3.executeQuery("SELECT form6_no,desg,emp,status,remarks,DATE_FORMAT(f_date,'%d/%m/%Y') FROM form6_internet_item WHERE form6_no = '"+form_no+"' ORDER BY id ASC   ");
						while(rs3.next()){
