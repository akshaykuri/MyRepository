...
function link(form_no)
    ...
    var f=document.form;    
    f.method="post";    
    f.action='NExASformReportDetails.jsp?form_no='+form_no;;    
    f.submit();
    }
</script>
...
<%!
	 ...
	 %>
     <%
     req = request.getParameter("nexas");
     req2 = request.getParameter("dateNE1");
     req3 = request.getParameter("dateNEE1");
...
NExAS REPORT</font></b></center></td></tr>
</table>
	   ...
	   <tr>
	      ...
	      <td bgcolor="white" width="100" align=center style="vertical-align:middle" ><font size="2"><b>REQUEST TYPE</b></font></td>
				            ...
				            if(req!=null && req.equals("all"))
				                ...
				                rs2 = st2.executeQuery("SELECT * FROM  form12_nexas WHERE form_date >= '"+dateF+"' AND form_date<='"+dateFF+"'  ORDER BY id ASC   ");
				                }
				           else
				               {
				        	   System.out.println("Specific BRANCH "+req);
				        	   rs2 = st2.executeQuery("SELECT *  FROM  form12_nexas WHERE req_Branch='"+req+"' AND form_date >= '"+dateF+"' AND form_date<='"+dateFF+"'  ORDER BY id ASC   ");
				               }
				           while(rs2.next())
				                 {
				            	 flag=1;
