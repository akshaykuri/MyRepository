...
<table border="1"  align="center" bgcolor="#F0F0F0" bordercolor="#74c1b9">
	<tr>
	  ...
	  <td bgcolor="#74c1b9" width="120" align=center><font color="black">No of Users</font></td>
 ...
 try 
  ...
  //String query = "select assettype,configname from assetconfgfield where assettype='"+an+"' and configname ='"+co+"'";
   ...
   //rs = st.executeQuery(query);
    ...
    String query1 = "select distinct u.id,u.license_no,u.user,u.ip_addr,s.license_type,s.No_of_users from software_details_multiple u inner join software_details s on s.license_no=u.license_no where s.license_no='"+no+"'";
   st2 = conn.createStatement();
   rs2 = st2.executeQuery(query1);  %>
   <%while(rs2.next()){%>
    <tr>
         <%--  <td style="visibility: hidden;"><input type="hidden" name="sid" value="<%=sid%>"></td> --%>
         <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("license_type")%>"></td>
         <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("No_of_users")%>"></td>
   	     <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("user")%>"></td>
	      <td bgcolor="#d2f7f3"><input name="ip" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("ip_addr")%>"></td>
