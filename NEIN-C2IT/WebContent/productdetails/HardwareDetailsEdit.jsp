...
<form name="form"  method="post" enctype="multipart/form-data" action="HardwareDetailUpdate.jsp" >
...
System.out.println("edit asset----"+swid);
 ...
 try 
  {
    ...
    String query1 = "select s.hw_id,s.Asset_no,s.amc_po,s.amc_vender,s.branch,s.sub_branch,s.amc_from_date,s.amc_to_date,s.amc_exp_date,s.amc_amount,b.m_b_name,sb.b_name from hardware_details s inner join branch_master_main b on b.m_b_no=s.branch inner join branch_master sb on sb.id=s.sub_branch  where s.hw_id='"+swid+"'";
   st2 = conn.createStatement();
   rs2 = st2.executeQuery(query1);  %>
   <tr>
   <%while(rs2.next()){ %>
    
         <td style="visibility: hidden;"><input type="hidden" name="sid" value="<%=rs2.getInt("hw_id")%>">
         <input type="hidden" name="assetno" value="<%=rs2.getString("Asset_no")%>">
         <input type="hidden" name="amcpo" value="<%=rs2.getString("amc_po")%>">
         </td>
...
String query2 = "select hwc_id,Asset_no,amc_po,hw_invoice from hardware_amc_invoice where amc_po='"+rs2.getString("amc_po")+"'";
...
<input type="hidden" name="invoice" id="invoice" value="<%=rs3.getString("hw_invoice")%>" >
 <a href="DownloadFile.jsp?f=<%=rs3.getString("hw_invoice")%>" >VIEW/DOWNLOAD</a>


<a href="hwInvoiceDel.jsp?f=<%=rs3.getString("hwc_id")%>&g=<%=rs2.getInt("hw_id")%>" >Delete</a>
</font>
