...
//psmt = cn2.prepareStatement("UPDATE form4_accesscard_item SET status=?,remarks=?,f_date=?,presence=? WHERE form4_no=? AND desg=? AND emp=? ");
  psmt = cn2.prepareStatement("UPDATE form4_accesscard_item f,(SELECT max(id) as idf FROM form4_accesscard_item WHERE form4_no=? AND desg=? AND emp=?) as p SET f.status=?,f.remarks=?,f.f_date=?,f.presence=? WHERE f.form4_no=? AND f.desg=? AND f.emp=? AND f.id=p.idf");
...
psmt2 = cn2.prepareStatement("UPDATE form4_accesscard SET ITusername=?,ITpassword=?,ITcreated=?,ITremark=? WHERE form4_no=?");
...
{
postMail.postMail(i_email_id,i_name,"pass1234",mailingCheck[u],"ACCESS CARD FORM :  " +form_no+ " ",i_name+  " "+  "request is Approved " +form_no+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
}
