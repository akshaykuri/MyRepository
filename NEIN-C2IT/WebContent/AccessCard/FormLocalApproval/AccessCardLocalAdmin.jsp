 ...
 */
  ...
  <script language="javascript" type="text/javascript"> 
  function resendMail(email_id,form4_no,mail,name)
  		 {
	 if (typeof XMLHttpRequest != "undefined"){    
         xmlHttp= new XMLHttpRequest();    
           }      else if (window.ActiveXObject){
           alet("Creating html http object");    
             xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                    ...
                    alert("Browser does not support XMLHTTP Request");    
                      return;     
                       }       var url="../../ResendMailAccessCard?mail="+email_id+","+form4_no+","+mail+","+name+"";  
...
<form  name="form" method="post"  action="AccessCardLocalAdminBack.jsp">
...
<table id="tableData" name="tableData" width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=email_id%>','<%=form4_no%>','<%=m%>','<%=SessionName%>');" /></td> 
...
%> 
<%String flag = c2ITUtil1.getAuthorizedNameAccessCard(form4_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),emp_no1); %>
