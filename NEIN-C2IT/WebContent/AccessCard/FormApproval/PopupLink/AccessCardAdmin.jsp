     ...
     if (document.getElementsByName("mmdradio")[0].checked)
       ...
       if (document.getElementsByName("mmdradio")[0].checked && document.getElementsByName("higherAuthority")[1].checked )
           ...
           if(itname.value==""||itname.value==null)
   		         {
   		         alert("Please Enter Access card no. ");
   		         itname.focus();
   		         return false;
   		         }
 ...
 */
 ...
 function hideBcc2()
 {
  ...
  <script language="javascript" type="text/javascript"> 
  function resendMail(mail,form1_no,email_id,sessionName)
  		 {
	if (typeof XMLHttpRequest != "undefined"){    
         xmlHttp= new XMLHttpRequest();    
           }      else if (window.ActiveXObject){
           alet("Creating html http object");    
             xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                    ...
                    alert("Browser does not support XMLHTTP Request");    
                      return;     
                       }       var url="../../../ResendMailAccessCard?mail="+mail+","+form1_no+","+email_id+","+sessionName+"";  
...
<form  name="form" method="post"  action="AccessCardAdminBack.jsp">
...
//String a_model=null,a_data_card=null,a_email=null,a_newins=null,a_ACCESS_CARD=null,a_mobile_model=null,a_mobile_no=null,a_access_card=null,a_drive=null,a_other_asset=null,a_remarks=null;
//String r_model=null,r_data_card=null,r_email=null,r_newins=null,r_ACCESS_CARD=null,r_mobile_model=null,r_mobile_no=null,r_access_card=null,r_drive=null,r_other_asset=null,r_remarks=null;

//form2_ACCESS_CARD
String form4_no=null,f_date=null,idextend=null,branch=null,depart=null,full_name=null,designation=null,scopeOfWork=null,emp_code=null,reporting_officer=null,requestOption=null,requestBy=null,remarks=null;
...
rs100 = st100.executeQuery("select * from form4_accesscard WHERE form4_no='"+req+"'"); 
while(rs100.next())
{
	    form4_no = rs100.getString("form4_no");
...
<table width="101%"  cellspacing="0" cellpadding="0" bgcolor="grey" align="center">
	<tr>
		...
		<input type="hidden" name="form4_no" value="<%=form4_no%>" />
...
<td style="text-align:left;" bgcolor="grey"><input name="form4_no" value="<%=form4_no%>" size="31" readonly="readonly"/> </td>
</tr>
<tr>
...
<table id="tableData" name="tableData" width="100%" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  String emp_no="",levelno="";
	  rs1 = st1.executeQuery("select form4_no,desg,emp,status,remarks,f_date,mail,empid,levelno from form4_accesscard_item WHERE form4_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
		  String form4_noD= rs1.getString("form4_no");
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=m%>','<%=form4_no%>','<%=email_id%>','<%=SessionName%>');" /></td> 
...
%>  
<%String flag = c2ITUtil1.getAuthorizedNameAccessCard(form4_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("EmpNo").toString()); %>
   ...
   <tr style="display: none;" id="idApproved" >
	...
	String dbName = dbProperties.getString("csdb.dbName");
     ...
     String maxlevel="";
	  ...
	  String i_b_no1=form4_no.substring(14,16);
	  
	  
	  
      rs12= st12.executeQuery("SELECT max(levelno) FROM formapproval WHERE branch='"+i_b_no1+"' and formtype='ACCESS_CARD'  ORDER BY levelno ASC");
     if(rs12.next())
         { 
    ...
    rs11 = st11.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+i_b_no1+"' and empid='"+emp_no+"' and formtype='ACCESS_CARD'");
     if(rs11.next())
         { 
     ...
     /* ////////////////////////// Skeeping level code  ////////////////////////////////// */
      ...
      rsnext = stnext.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='ACCESS_CARD' ORDER BY levelno ASC");
      if(rsnext.next())
           {
     ...
     if(allopt.equals("0") || allopt.equals(""))
    	  ...
    	  if(!levelno.equals(maxlevel) && !dirappro.equals("1"))
    	     ...
    	     rs5 = st5.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='ACCESS_CARD' ORDER BY levelno ASC");
    	     if(rs5.next())
                {
               ...
               %><input type="hidden" name="senddirect" id="senddirect" value="sendonly" readonly/>
               ...
               else
                  {
            	   ...
            	   if(!levelno.equals(maxlevel) && dirappro.equals("1"))
            	         ...
            	         <td style="display: none;" id="send">Send to HO(IT) for Create ACCESS_CARD<br>
                           ...
                           <td style="display: none;" id="direct">Create ACCESS_CARD<br>
            	           <table id="idFinal" align="center" width="100%" border="1" style="height: 150;" cellspacing="0" cellpadding="0" bgcolor="lightgrey" >
                               <tr>
<td class="it2" style="text-align: right;width:23%;"> Access Card No. :</td>
<td class="it2"  style="text-align:left;width:25%;"> <input type="text" name="ITusername" id="uname" maxlength="65" /></td>
</tr>

<tr>
<td class="it2" style="text-align: right;width:23%;">Issued Date:</td>
<td class="it2"  style="text-align:left;width:25%;"> 

<input type="text" name="ITpassword" id="pwd" maxlength="65" value="<%=po_date.format(new java.util.Date())%>" readonly="readonly"/></td></tr>
