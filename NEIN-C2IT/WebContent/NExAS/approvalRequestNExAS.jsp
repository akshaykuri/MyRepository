...
function OtherAdmin()
    ...
    document.form.action="approvalRequestNExASBack.jsp";
...
<table width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  rs1 = st1.executeQuery("select form_no,desg,emp,status,remarks,form_date,mail,levelno from form12_nexas_item WHERE form_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
...
<%String flag = c2ITUtil1.getAuthorizedNameNExAS(form2_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("EmpNo").toString()); %>
...
//rs5 = st5.executeQuery("Select * from desg");emp_no,levelno
rsl1 = stl1.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+req_Branch+"' and empid='"+session.getAttribute("EmpNo").toString()+"' and formtype='NExAS_NEWINS'");
if(rsl1.next())
    { 
  ...
  <tr style="visibility: hidden;display: none;" id="idApproved" >
   <td>
       <%if(!levelno.equals(maxlevel) && !levelno.equals("over"))
             ...
             &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input align="left" type="radio" name="higherAuthority" value="directApprove" id="idmNon" onclick="showHigherTR();"/>Direct Approve1(Mail will go to HOIT for further process)
            <%}%> 
   ...
   <% 
     ...
     if(allopt.equals("0"))
           {
    	   rs5 = st5.executeQuery("SELECT * FROM formapproval WHERE branch='"+req_Branch+"' and levelno='"+nextlevel+"'  ORDER BY levelno ASC");
    	   if(rs5.next())
                {
