...
String opRange = request.getParameter("costOPRange");
...
//System.out.println("Optional menu :"+opval);
...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form12_nexas WHERE form_no IN(SELECT form_no FROM form12_nexas LEFT JOIN branch_master b ON(substr(form_no,12,2)=b.b_no) WHERE b.b_no=? GROUP BY form_no)");
...
int idextend = id;
psmt = cn2.prepareStatement("INSERT INTO form12_nexas(form_no,idextend,empid,full_name,depart,designation,r_officer,empmail,emp_mob,requestBy,requestOption,req_Branch,empBranch,curr_NExASID) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form12_nexas_item(form_no,desg,emp,status,remarks,mail,empid,levelno) VALUES(?,?,?,?,?,?,?,?)");
...
String msg = "NExAS__FORM__SAVED__AND__SENT__SUCCESSFULLY";
