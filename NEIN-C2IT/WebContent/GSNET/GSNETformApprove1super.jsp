...
var l,flag21=0;
chks21 = document.getElementsByName('role[]');
//alert("herer..1.......");
for (l = 0; l < chks21.length; l++)
  {
   if (chks21[l].checked)
   {
   flag21=1; 
...
function OtherAdmin()
    ...
    document.form.action="GSNETformApproval1Back.jsp";
...
while(rs100.next())
	    ...
	    currNExASID = rs100.getString("curr_GSNETID");
...
<%if(!requestOption.equals("New")) { %>
    <tr>
        <td  style="text-align: right;">Current GSNET ID:</td> 
        <td style="text-align:left;">
...
<table width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  String statusD=null;
	  rs1 = st1.executeQuery("select form_no,desg,emp,status,remarks,form_date,mail,empid,levelno from form15_gsnet_item WHERE form_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
...
System.out.println("Seeempid"+sessempid);
 ...
 String flag = c2ITUtil1.getAuthorizedNameGSNET(form2_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("emp_no").toString()); %>
...
<input type="hidden" id="idstatus" value="<%= flag %>" />
  ...
  <% ResourceBundle dbProperties = ResourceBundle.getBundle("csdb");
    ...
    // System.out.println("Max Level no :"+maxlevel);
     ...
     //rs5 = st5.executeQuery("Select * from desg");emp_no,levelno
     rsl1 = stl1.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+req_Branch+"' and empid='"+sessempid+"' and formtype='GSNET'");
     if(rsl1.next())
         { 
