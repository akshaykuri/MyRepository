...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form7_newins WHERE form7_no IN(SELECT form7_no FROM form7_newins LEFT JOIN branch_master b ON(substr(form7_no,16,2)=b.b_no) WHERE b.b_no=? GROUP BY form7_no)");
...
if(rs_id.next()){
	...
	try
	{
	...
	catch(SQLException ex)
	    {
		 ...
		 new_id = "NEIN/CS/NEWINS/"+i_b_no+"-1";
	    }
...
//40
psmt = cn2.prepareStatement("INSERT INTO form7_newins(form7_no,f_date,idextend,branch,depart,full_name,designation,scopeOfWork,emp_code,reporting_officer,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,form_name,deleteremarks,empid,requBranchCode) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt.setString(23,"Newins User Request");
psmt.setString(24,deleteremarks);
psmt.setString(25,empid);
psmt.setString(26,reqbrcode);
...
psmt2 = cn2.prepareStatement("INSERT INTO form7_newins_item(form7_no,desg,emp,status,remarks,f_date,mail) VALUES(?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form7_newins_item(form7_no,desg,emp,status,remarks,f_date,mail,presence) VALUES(?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageNewins(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail," NEWINS FORM :  " +new_id+ " ",i_name+  " "+  "request you to approve - Newin's User : FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);              
...
response.sendRedirect("NewinsPage.jsp?msg=" +msg);
