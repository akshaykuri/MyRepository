...
{  var p1=document.form.req;
 if ((p1.value==null)||(p1.value=="NULL"))
				{alert("Select NEWINS Number!"); p1.focus(); return false; }
...
document.form.action="NewinsAdmin.jsp?msg=1";
...
document.form2.action="NewinsLocalAllAdmin.jsp?msg=1";
...
<form name="form" method="post" action="NewinsAdmin.jsp?msg=1">
...
<%
	 ...
	 System.out.println("Admin Name : For selection : "+AdminName);
	 desg = (String) session.getAttribute("desg");
	 System.out.println("Admin Desg : For selection : "+desg);
	%>	 
<%
String tot_val = null;
Connection cn9 = null;
...
//rs9 = st9.executeQuery("SELECT DISTINCT form7_no FROM form7_newins_item WHERE emp='"+AdminName+"' and desg='"+desg+"' and status='Pending' AND form7_no LIKE 'NEIN/CS/NEWINS%' ORDER BY form7_no ASC");
rs9 = st9.executeQuery("SELECT DISTINCT form7_no FROM form7_newins WHERE Approval_emp='"+AdminName+"' and Approval_desg='"+desg+"' and Approval_sttatuss='Pending' AND form7_no LIKE 'NEIN/CS/NEWINS%' ORDER BY form7_no ASC");
...
<form name="form2" method="post" action="NewinsLocalAllAdmin.jsp?msg=1">
<%-- %>	 <form action="POMonthWiseBack.jsp"> 
	<form action="POWiseBack.jsp">--%>
...
//rs9 = st9.executeQuery("Select DISTINCT substr(pordernon,6,2) from po ORDER BY substr(pordernon,6,2) ASC");
/* rs9 = st9.executeQuery("SELECT DISTINCT form7_no FROM form7_newins  ORDER BY form7_no"); */
//rs99 = st99.executeQuery("SELECT DISTINCT form7_no FROM form7_newins WHERE i_b_no='"+b_no+"' AND form7_no LIKE 'NEIN/CS/NEWINS%' ORDER BY idextend DESC");
rs99 = st99.executeQuery("SELECT DISTINCT form7_no,full_name FROM form7_newins WHERE form7_no LIKE 'NEIN/CS/NEWINS%' ORDER BY id DESC");
