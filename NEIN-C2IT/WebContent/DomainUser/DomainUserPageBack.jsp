...
psmt1001 = cn2.prepareStatement("SELECT max(idextend) FROM form2_domain WHERE form2_no IN(SELECT form2_no FROM form2_domain LEFT JOIN branch_master b ON(substr(form2_no,16,2)=b.b_no) WHERE b.b_no=? GROUP BY form2_no)");
...
if(rs_id.next()){
	...
	try
	{
	...
	catch(SQLException ex)
	    {
		 ...
		 new_id = "NEIN/CS/Domain/"+req_Branch+"-1";
	    }
...
//40
psmt = cn2.prepareStatement("INSERT INTO form2_domain(form2_no,f_date,idextend,branch,depart,full_name,designation,scopeOfWork,emp_code,reporting_officer,requestOption,requestBy,remarks,i_name,i_desg,i_city,i_b_no,i_admin,i_email_id,desg,emp,mail,form_name) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
psmt.setString(23,"Domain User Request");
...
psmt2 = cn2.prepareStatement("INSERT INTO form2_domain_item(form2_no,desg,emp,status,remarks,f_date,mail,empid,levelno,presence) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
psmt3 = cn2.prepareStatement("INSERT INTO form2_domain_item(form2_no,desg,emp,status,remarks,f_date,mail,empid,levelno,presence) VALUES(?,?,?,?,?,?,?,?,?,?)");
...
message = mailMessage.getMailMessageDomain(new_id.trim());
postMail.postMail(i_email_id,i_name,"pass1234",mail," Domain User FORM :  " +new_id+ " ",i_name+  " "+  "request you to approve - Domain User FORM. " +new_id+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);              
...
response.sendRedirect("DomainUserPage.jsp?msg=" +msg);
