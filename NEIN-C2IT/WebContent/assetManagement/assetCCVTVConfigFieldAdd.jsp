...
<body onload="myFunction()">
	<form method="post" action="asmInvCCTVDB.jsp" 
		name="form1" id="form"  >
		<input type="hidden" id="idflag" name="idflag" value="0" />
		<%
			System.out.println("------------- Asset CCTV Config File  -----------------");
			String asset_no = request.getParameter("asset_no");
		 	...
		 	int  CameraCount= Integer.parseInt(request.getParameter("NO_OF_CAMERA"));
		...
		<input type="hidden" id="cameraCount" name="cameraCount" value="<%=CameraCount%>"	/>
		...
		<div class="modal-content">
			<div class="modal-header">
				<center>
					<h2>IT ASSET INVENTORY FORM FOR CCTV SETUP</h2>
				</center>
			</div>
					...
					<table width="75%" border="1" style="height: 150;" cellspacing="0"
						...
						<tr>
							<td colspan="10"><center>
									<font size="5" face="Swiss" id="hiderow">Enter DVR
										Details</font>
								</center> 
							</td>
						</tr>
						<tr>
							...
							<td><font size="2" face="Swiss" id="hiderow"><font
									color="red" size="4">*</font>Waranty</font>
							</td><td><font size="2" face="Swiss" id="hiderow"><font
									color="red" size="4">*</font>Waranty ExpDate</font>
							</td>
						</tr>
						...
						<%
							...
							try {
								...
								con21 = DriverManager.getConnection(url21 + db21, userName21,
										password21);
								String query21 = "SELECT * FROM `asset_item` WHERE Asset_No='"+asset_no+"'and Inventory_No='"+Inventory_no+"'";
								...
								while (rs21.next()) 
								{
											...
											%>
										 	 <tr>
													...
													<td><input type="text" id="assetName" name="assetName" value="DVR" readonly ></td>
													...
													<td> <input type="text" name="dvrasset" id="dvrasset" value="DVR"/></td>
													<td> <input type="text" name="dvrserial" id="dvrserial" value=""></td>
													<td> <input type="text" name="dvrmodel" id="dvrmodel" value=""></td>
													<td> <input type="text" name="dvrbrand" id="dvrbrand" value=""></td>
										    		
										    		<td>
															<%
																Connection con222 = null;
																String url222 = "jdbc:mysql://localhost:3306/";
																String db222 = "csdb";
																String driver222 = "com.mysql.jdbc.Driver";
																String userName222 = "root";
																String password222 = "";
																// int sumcount=0;
																Statement st222 = null;
																ResultSet rs222 = null;
																try {
																	// id="branchName"
																	Class.forName(driver222).newInstance();
																	con222 = DriverManager.getConnection(url222 + db222,
																			userName222, password222);
																	String query = "select * from branch_master where b_no  ORDER BY b_name";
																	st222 = con222.createStatement();
																	rs222 = st222.executeQuery(query);
																	%> <select name="dvrbranchName" id="dvrbranchName">
																		<option selected="selected">Select</option>
																		<%
																			while (rs222.next()) {
																		%>
																		<option value="<%=rs222.getString(3)%>"><%=rs222.getString(2)%></option>
																		<%
																			}
																		%>
																								</select> <%
																...
																<td><select name="dvrwar" id="dvridWar"
																	onclick="CalExp(this,'dvridWarExpDate')">
											        		...
											        		<td><input type="text" style="width: 6em"
																name="dvridWarExpDate" value="" id="dvridWarExpDate">
															</td>
										    		...
										    		<tr>
														<td colspan="10"><center><font size="5" face="Swiss" id="hiderow">Enter Camera Details</font></center>														</td>
													</tr>
										    		<% for(int x=1;x<=CameraCount;x++)
													{
										    		  %>
											    	  <tr>
											    		<td><font size="2" face="Swiss" id="hiderow"><%=x%></font>	</td>
														<td><input type="text" id="assetName1" name="assetName1" value="CAMERA" readonly ></td>
														<td><input type="text"name="assetNo1" maxlength="99" id="assetNo1"							 placeHolder="please enter asset no" size=20 value="<%=asset_no%>" readonly /></td>
														<td> <input type="text" name="cameraasset" id="cameraasset" value="CAMERA"></td>
														<td> <input type="text" name="cameraserial<%=x%>" id="cameraserial<%=x%>" value=""></td>
														<td> <input type="text" name="cameramodel<%=x%>" id="cameramodel<%=x%>" value=""></td>
														<td> <input type="text" name="camerabrand<%=x%>" id="camerabrand<%=x%>" value=""></td>
														<td>
															<%
																Connection con = null;
																...
																try {
																	...
																	%> <select name="branchName<%=x%>" id="branchName<%=x%>">
																		<option selected="selected">Select</option>
																		<%
																			while (rs.next()) {
																		%>
																...
																<td><select name="idWar<%=x%>" id="idWar<%=x%>"
																	onclick="CalExp(this,'idWarExpDate<%=x%>')">
																	<option value="Select">Select</option>
																	<option value="0">0</option>
																	<option value="3">3 Months</option>
																	<option value="6">6 Months</option>
											        		...
											        		<td><input type="text" style="width: 6em"
																name="idWarExpDate<%=x%>" id="idWarExpDate<%=x%>">
															</td>
										    	</tr>	
											<%}
								}
