...
function validate() 
	...
	//  alert(conf[i]); 
	 ...
	 // alert("Field :"+field); 
	  if(field=="")
		  {
		  	...
		  	alert("Please Enter the All config value");
	        return false;
		  }
	  else
	  ...
	  //alert("All Configure :"+ allconf);
	  //alert("Please enter "+field+" "+"value" );
	  /* if(field=="")
		  {
		  alert("Please enter "+conf[i]+" "+"value" );
		  document.getElementById(idatype).value=atype;
		  break;
		  } */
	  }
	  
	  alert("asset type :: "+atype);
	 
	  if(atype == "DESKTOP")
 	 {
  		alert("Inside If Desktop ::"+atype);
  		var f=document.form;  
	 	f.method="post";
   		f.action='assetDesktopConfigFieldAdd.jsp?allconf='+allconf+'&atype='+atype+'&inventory_no='+inventory_no+'&asset_no='+asset_no;
   		f.submit();
 	 }
  	 else if(atype == "CCTV_SETUP")
  		 {
  			alert("Inside else if part CCTV :"+atype);
  		 ...
  		 {
  			alert("Inside else IF  part UPS_SETUP: " + atype);
 			var f=document.form;  
	 	   	f.method="post";
			f.action='assetUPSConfigFieldAdd.jsp?allconf='+allconf+'&atype='+atype+'&inventory_no='+inventory_no+'&asset_no='+asset_no;
	 		 ...
	 		 alert("Inside else part "+atype);
		 	 var f=document.form;  
		     f.method="post";   
			 f.action='assetConfigAddBack.jsp?allconf='+allconf;
			 f.submit();
	      } 
      }
</script> 
...
<br>
	 ...
	 <table width="100%" border="1" style="height: 150; border: 1px solid black; padding: 5px;" cellpadding="0" bgcolor="lightgrey">
	   	    ...
	   	    <%
	   	 System.out.println("------------- Asset Config Add  -----------------");
