...
<%
System.out.println("------------- audit  asset Available for name -----------------");
String uid=request.getParameter("AssetType");  
String val1=request.getParameter("val");
System.out.println("Asset name: "+uid +"  value :"+val1 );
...
String password = "";
String id1="auditidaname"+val1;
String name1="auditidaname"+val1;
...
String  buffer="";//;="<select name='"+name1+"' id='"+id1+"' class='auditastno'><option  value = '-1'>Select</option>";
try{ 
        ...
        ResultSet rs = stmt.executeQuery("select * from asset_item where (Asset_no='" + uid + "'); ");
        // String buffer = "";
        while(rs.next()){
        System.out.println(" asset name ::: "+rs.getString(19)); 
        	//buffer=buffer+"<option value='"+rs.getString("EMP_NAME")+"'>"+rs.getString("EMP_NAME")+"</option>";
        	 buffer=buffer+"<input type='text' id='p1' name='auditAssetName' maxlength='99'  class='asname' value='"+rs.getString(19)+"' />";
        	 buffer=buffer+"<input type='hidden' id='configuration' value=' " + rs.getString(19)+"'>";
