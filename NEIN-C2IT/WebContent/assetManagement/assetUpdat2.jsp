...
function editRecord(id)
    {  
		...
		/* if(document.getElementById("idatatus").value==0)
			...
			var f=document.form;    
	    	f.method="post";    
	    	f.action='assetUpdate.jsp?id='+id+'&id1='+id1;    
	    	f.submit();
      // }
    }
</script>
...
<script type="text/javascript">
function updateBranch(str,second){  
	//alert(str);
      ...
      xmlHttp= new XMLHttpRequest();    
          ...
          xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                 ...
                 alert("Browser does not support XMLHTTP Request");   
                   return;     
                    }       var url="updateBranchfeatch.jsp"; 
                    //alert (str);
                            url +="?bno=" +str+"&idbr="+second;   
                           // alert(url);
                           xmlHttp.onreadystatechange = branch;  
                    	 ...
                    	 var idbr;
             	         ...
             	         if(xmlHttp.readyState==4 || xmlHttp.readyState=="complete")//iddesg,iddept,idbranch,idcono
                                 { 
	                                 ...
	                                 idbr=document.getElementById("p2").value;
	                                 document.getElementById(idbr).value=document.getElementById("p1").value;
                                 }
                           } 
</script>
...
<%-- <table id="invoicetable" width="1000" bgcolor="" cellpadding="4" cellspacing="6" align="center" border=1>
	...
	<td   height="21" bgcolor="lightgrey" style="font-size: 1" width="10" align="center"><font size="2"> ADMIN DESG:<%=session.getAttribute("desg")%><b></b></font></td>
                ...
                <table class="dynatable" width="1000" border="" style="height: 150;" cellspacing="0" cellpadding="0" bgcolor="lightgrey" align="center">
                  <tr align="center">
                                 <td bgcolor="gray" colspan="8"><strong><font size ="4" color="white">UPDATE ASSET DETAILS</font></strong></td></tr>			   
                  <%
			      try
	                {
	                ...
	                %>
	                  ...
	                  <% query1 = "select Inventory_No,Asset_No,AssetType,Branch,BranchName,WarExpDate,ConfigField,assignStatus,Scrap,Maintanance,Assignedfor,assignedbraemp from  asset_item WHERE (Asset_No='"+ano+"' and AssetType not in ('DESKTOP','CCVTV','UPS_SETUP'))";                                      
	    			  ...
	    			  if(asstatus.equals("1") && Asfor.equals("USER"))
	    			         {
	    				     query2 = "select  EmpId,EmpName,empbranch,AssignedByName from  assignotherassetemp WHERE (ano='"+ano+"'and AssetType not in ('DESKTOP','CCVTV','UPS_SETUP'))"; 
	    				     ...
	    				     while(rs2.next())
  			                 ...
  			                 {
	    				     query2 = "select BranchCode,BranchName,BranchAddress,AssignedByName from  assignassetoffice WHERE (AssetNo='"+ano+"'and AssetType not in ('DESKTOP','CCVTV','UPS_SETUP'))"; 
	    				     psmt2 = conn.prepareStatement(query2);
	                         rs2 = psmt2.executeQuery();
	    				     while(rs2.next())
	                            {
	    			 ...
	    			 String query3 = "select m_b_no from  branch_master_main WHERE m_b_no in(90,10,20,30,40)"; 
  				     psmt3 = conn.prepareStatement(query3);
                     rsbr = psmt3.executeQuery(); 
	    			  %>
			          ...
			          <td width="67" align="center"  height="21" ><font size="2" ><b>Configuration</b></font></td>
	                   ...
	                   <tr style="background-color:white;">
		                ...
		                <input type="hidden" name="ano" maxlength="30" id="idano" value="<%=ano%>" size=20 />
		                <td align=center><font color="black" size="2"><%=atype %></font></td>
		                <input type="hidden" name="atype<%=k%>" maxlength="30" id="idatype<%=k%>" value="<%=atype%>" size=20 />
		                <td align=center>
		                <select name="brcode<%=k%>" id="idbrcode<%=k%>" onchange="updateBranch(this.value,'bname<%=k%>')" style="font-size: 11px;width:50px; overflow: auto; height: 20px; background-color:red; ">
		                     <option value="<%=bran%>"><%=bran%></option>
		                     <%
		                     while(rsbr.next())
		                         { if(rsbr.getInt(1)!= Integer.parseInt(bran)){%>
		                    	    <option value="<%=rsbr.getInt(1)%>"><%=rsbr.getInt(1)%></option>
		                    <%}}%>
		                </select>
		                 </td>
		                <td align=center><input type="text" value="<%=brname%>" id="bname<%=k%>" name="bname<%=k%>" readonly></input></td>
		              ...
		              <tr style="background-color:white;">
		                <td align=center><font color="black" size="2"><%=asstatus%></font></td>
		                 <td align=center>
		                  <select name="scr<%=k%>" id="idscr<%=k%>" style="font-size: 11px;width:50px; overflow: auto; height: 20px; background-color:red; ">
		                  ...
		                  <td align=center>
		                  <select name="main<%=k%>" id="idmain<%=k%>" style="font-size: 11px;width:50px; overflow: auto; height: 20px; background-color:red; ">
		              ...
		              <tr>
		                 <td style="font-size: 1" colspan=8>
		                     <center>
		                         <input style="size: 1;" type="button" title="Back To Selection Date Page" value="UPDATE" onclick='editRecord("<%=rs.getString(1)%>,<%=rs.getString(2)%>,<%=rs.getString(3)%>,<%=rs.getString(4)%>,<%=rs.getString(5)%>,<%=rs.getString(6)%>,<%=rs.getString(8)%>,<%=rs.getString(9)%>,<%=rs.getString(12)%>,<%=empid%>,<%=empnm%>,<%=empbra%>,<%=Asfor%>,<%=k %>");' />
...
<a href="assetUpdate1.jsp?msg=1"><img src="../bakk.png" border="0" height="35" width="35"></a><address><font size="2">BACK</font></address> 
</td> 
</tr> 
</table>                              
<table width="1118" align="center" border="0" cellspacing="0" cellpadding="0">
