...
<script language="javascript"   type="text/javascript" src="../../../js/onSubmitWait.js"></script>
...

 {
   ...
   if(chkvalue == "FinalStatus")
	 	{
	 	    /*  // alert("SELECT SECOND ");
	 	      var ddd = document.form.ITremark;
			  ...
			  if(chkvalue=='')
		       ...
		       if ((ddd.value==null)||(ddd.value=="NULL")||(ddd.value==""))
				 {
			       alert("Enter Remarks !");
			       ddd.focus();
			       return false;
...
function resendMail(mail,form11_no,email_id,sessionName)
  		 {
         ...
         xmlHttp= new XMLHttpRequest();    
           ...
           alet("Creating html http object");    
             xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                    ...
                    alert("Browser does not support XMLHTTP Request");    
                      return;     
                       }       var url="../../../ResendMailDSC?mail="+mail+","+form11_no+","+email_id+","+sessionName+"";  
...
<form  name="form" method="post"  action="DSCAdminBack.jsp">
...
System.out.println("desg Number POP UPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP:"+Sessiondesg);
...
String i_name=null,i_desg=null,i_city=null,i_b_no=null,i_admin=null,i_email_id=null,desg=null,emp=null,mail=null,cin_no=null;
String req = request.getParameter("rss");
System.out.println("REQ RESULT : "+req);
...
while(rs100.next())
{
	    ...
	    cin_no = rs100.getString("select_cin");
...
<tr>
&nbsp;&nbsp;&nbsp; <td  style="text-align: right;width: 215px; "  >CIN Number :</td>
<td style="text-align:left;"><input name="cin_no" value="<%=cin_no%>" size="31" readonly="readonly"/> </td>
...
<table id="tableData" name="tableData" width="100%" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  String statusD=null;
	  rs1 = st1.executeQuery("select id,form11_no,desg,emp,status,remarks,f_date,mail from form11_dsc_item WHERE form11_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=m%>','<%=form11_no%>','<%=email_id%>','<%=SessionName%>');" /></td> 
...
psmt.setString(1,"90");
psmt.setString(2,"ASST.MANAGER IT");
...
System.out.println("FLAG value --------------------- : "+flag);
		...
		//if((ITbno.equals(session.getAttribute("b_no")) && ITdesg.equals(session.getAttribute("desg"))))
		if(SessionSuperAdmin!=null && SessionSuperAdmin.equals("5") && Sessiondesg!=null && Sessiondesg.equals("SR.MANAGER IT"))
	{
%>
<br />
<table >
...
<br />
		...
		cn11 = con.getConnection2();
		  ...
		  rs11 = st11.executeQuery("select DISTINCT emp,form11_no,desg,emp,status,f_date,mail from form11_dsc_item WHERE form11_no='"+req+"' ORDER BY id ASC"); 
		  while(rs11.next())
		  {
