...
psmt = cn2.prepareStatement("UPDATE form2_domain_item f,(SELECT max(id) as idf FROM form2_domain_item WHERE form2_no=? AND desg=? AND emp=?) as p SET f.status=?,f.remarks=?,f.f_date=?,f.presence=? WHERE f.form2_no=? AND f.desg=? AND f.emp=? AND f.id=p.idf");
//psmt = cn2.prepareStatement("INSERT INTO form2_domain_item(status,remarks,f_date,presence,form2_no,desg,emp) VALUES(?,?,?,?,?,?,?)");
...
psmt10.setString(5,i_name);
psmt10.setString(6,i_desg);
psmt10.setString(7,"Prasanna Kumar");
psmt10.setString(8,"MANAGER");
...
{
postMail.postMail(i_email_id,i_name,"pass1234",mailingCheck[u]," DOMAIN USER FORM :  " +form_no+ " ",i_name+  " "+  "request is Approved " +form_no+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
}

message = null;

msg = "DOMAIN__USER__FORM__NO__:__'"+form_no+"'__SAVED__AND__SENT__SUCCESSFULLY";
