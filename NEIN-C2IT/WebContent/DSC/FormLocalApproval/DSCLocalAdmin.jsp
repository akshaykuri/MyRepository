...
<!-- Help Page pop up starts  -->
 ...
 //  var rbb = document.form.requestOptionTypeDate;
   if (!document.getElementsByName("requestOptionType")[0].checked && !document.getElementsByName("requestOptionType")[1].checked) 
	 {
     alert("Select Radio To Specify DSC in good condition");
     document.getElementsByName("requestOptionType")[0].focus();
     return false;
  } 
  /*  if ((rbb.value==null)||(rbb.value=="NULL")||(rbb.value==""))
	{
	 alert("Enter DSC Received Date !");
	 rbb.focus();
	 return false;
	 } */
...
//document.form.action="DSCAdminBack.jsp";
document.form.action="DSCAdminLocalBack.jsp";
document.form.method="post";
document.form.submit();
}
function ITManager()
{
document.form.action="DSCAdminPrasannaBack.jsp";
document.form.method="post";
document.form.submit();
}
...
function yesnoCheck() {
	...
	if (document.getElementById('Yes').checked) {
		...
		document.getElementById('idmrapproved').style.display = 'block';
	} else if(document.getElementById('No').checked){
	    document.getElementById('ifYes').style.display = 'none';
	    document.getElementById('idmrapproved').style.display = 'none';
	}
	else {
	    document.getElementById('ifYes').style.display = 'none';
	}
...
function resendMail(email_id,form11_no,mail,name)
  		 {
	 if (typeof XMLHttpRequest != "undefined"){    
         xmlHttp= new XMLHttpRequest();    
           }      else if (window.ActiveXObject){
...
<form  name="form" method="post"  action="DSCAdminLocalBack.jsp">
...
//form11_dsc
String form11_no=null,f_date=null,idextend=null,branch=null,depart=null,full_name=null,designation=null,scopeOfWork=null,emp_code=null,reporting_officer=null,requestOption=null,requestBy=null,remarks=null;
...
rs100 = st100.executeQuery("select * from form11_dsc WHERE form11_no='"+req+"'"); 
while(rs100.next())
{
	    form11_no = rs100.getString("form11_no");
...
<table width="907"  cellspacing="0" cellpadding="0" bgcolor="grey" align="center">
	...
	DSC FROM EMPLOYEE</label>
		...
		<input type="hidden" name="form1_no" value="<%=form11_no%>" />
...
<td style="text-align:left;" bgcolor="grey"><input name="branch" value="<%=form11_no%>" size="31" readonly="readonly"/> </td>
</tr>
<tr>
...
<tr>
<td align="right">Received DSC in good condition ?  :</td>
<td  colspan="3"> 
     <input style="" type ="radio" name = "requestOptionType" onclick="javascript:yesnoCheck();" value="Yes" id="Yes"/>Yes
     <input style="" type ="radio" name = "requestOptionType" onclick="javascript:yesnoCheck();" value="No" id="No"/>No
     
     </td>

</tr>

     <tr id="ifYes" style="display:none;margin-top:3%;">
  
<td align="right">Received Date  :</td>
<td  colspan=""> 
    <!--  <input style="" type ="text" name = "requestOptionTypeDate" value ="" id=""/> -->
     <input type="text"  style="width: 6em"  name="requestOptionTypeDate" id="idCalendar" value="" onclick="showCalendarControl(this)" onkeypress="showCalendarControl(this)" readonly="readonly" />
...
<table width="895" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  String statusD=null;
	  rs1 = st1.executeQuery("select form11_no,desg,emp,status,remarks,f_date,mail from form11_dsc_item WHERE form11_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
		  String form11_noD= rs1.getString("form11_no");
	 ...
	 <%-- <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=m%>','<%=form11_no%>','<%=empD%>');" /></td>  --%>
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=email_id%>','<%=form11_no%>','<%=m%>','<%=SessionName%>');" /></td> 
...
%> 
<%String flag = c2ITUtil1.getAuthorizedNameDSC(form11_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString()); %>
...
%>
<table id="it" bgcolor="grey" width="895">
<tr><td id="ita">IT Dept Use Only</td></tr>
</table>

<table id="table3" bgcolor="grey" width="895">
<tr>
...
<br />
		...
		cn11 = con.getConnection2();
		  ...
		  rs11 = st11.executeQuery("select form11_no,desg,emp,status,remarks,f_date,mail from form11_dsc_item WHERE form11_no='"+req+"' ORDER BY id ASC"); 
		  while(rs11.next())
		  {
...
<table align="center" class="dynatable" width="100%" border="1" style="height: 150;" cellspacing="0" cellpadding="0" bgcolor="lightgrey">
 ...
 <input type="hidden" value="<%= session.getAttribute("desg") %>" id="iddesg"/>
  <input type="hidden" value="<%= session.getAttribute("form11_no") %>" id="idmrId"/>
<input type="hidden" id="idstatus" value="<%= flag %>" />

  
  <tr>
    <td align="left"><input align="left" type="radio" name="mmdradio" id="idmrapproved" value="Approved" onclick="return(showTR());"/>Approved 
 ...
 {%>
  ...
  <td>&nbsp;&nbsp;<!-- <a href="HelpPageCS.jsp" onclick="return popitup('HelpPageCS.jsp')" id="hiderow"><font size=2>HELP</font></a> -->
   	<input type="button" id="btnShowSimple" value="HELP" />
   	   ...
   	   <td style="font-size:1">&nbsp;&nbsp;<!-- <input style="size:1;font-size: 1.1em;" type="button"  value="BACK" onclick="window.location.href='DSCSelectAdmin.jsp?msg=1'"/> --> 
