...
<title>ACCESS CARD REPORT</title>
...
{   
	...
	if(confirm('You are above to delete a Access Card form'))
	{
var f=document.form;    
f.method="post";    
f.action='accessCardDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">ACCESS CARD FORM</td>
...
String query = "SELECT f.id,f.form4_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM  form4_accesscard f INNER JOIN  form4_accesscard_item g ON(f.form4_no=g.form4_no) WHERE Approval_status='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
