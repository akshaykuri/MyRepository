...
{   
	...
	if(confirm('You are above to delete a lotus form'))
	{
var f=document.form;    
f.method="post";    
f.action='lotusDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">LOTUS USER</td>
...
String query = "SELECT f.id,f.form3_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM  form3_lotus f INNER JOIN  form3_lotus_item g ON(f.form3_no=g.form3_no) WHERE Approval_status='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
