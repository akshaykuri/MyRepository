...
<jsp:useBean id = "postMail" class="CON2.PostMail" /> 
 ...
 System.out.println("DOMAIN DELETE "+id);
 ...
 {          
	 ...
	 rs = st2.executeQuery("SELECT id,form2_no FROM form2_domain WHERE id="+id+"");
	 if(rs.next()){
...
/* 	 int i=st.executeUpdate("delete from form2_domain where id="+id+"");    
	 int j=st3.executeUpdate("delete FROM form2_domain_item WHERE form2_no='"+FormNo+"'");  */
	 
	 int i=st.executeUpdate("UPDATE form2_domain SET deleted='yes' where id="+id+"");
...
/* 	 deleteBy.trim();
	...
	// postMail.postMail(deleteMail,deleteBy,"pass1234",mail_id," IT CLEARANCE FORM :  " +FormNo+ " ",deleteBy+  " "+  "Deleted IT CLEARANCE FORM :     "+FormNo+"");              
	 ...
	 response.sendRedirect("domain.jsp?msg="+msg);
	 }
	 catch (Exception e) {
