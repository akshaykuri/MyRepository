...
{   
	...
	if(confirm('You are above to delete a NEWINS form'))
	{
var f=document.form;    
f.method="post";    
f.action='newinsDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="7" align="center">NEWINS FORM</td>
...
String query = "SELECT f.id,f.form7_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM   form7_newins f INNER JOIN    form7_newins_item g ON(f.form7_no=g.form7_no) WHERE Approval_status='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
