...
{   
	...
	if(confirm('You are above to delete a Domain'))
	{
var f=document.form;    
f.method="post";    
f.action='domainDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">DOMAIN USER</td>
...
String query = "SELECT f.id,f.form2_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM form2_domain f INNER JOIN form2_domain_item g ON(f.form2_no=g.form2_no) WHERE Approval_status='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
