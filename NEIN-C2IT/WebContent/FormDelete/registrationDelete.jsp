...
<jsp:useBean id = "postMail" class="CON2.PostMail" /> 
 ...
 System.out.println("ACCESS CARD DELETE "+id);
 ...
 {          
	 ...
	 rs = st2.executeQuery("SELECT id,form10_no FROM form10_registration WHERE id="+id+"");
	 if(rs.next()){
...
/* 	 rs2 = st4.executeQuery("SELECT id,emp,form1_no,mail FROM form1_it_clearance_item WHERE form1_no="+FormNo+" ");
	...
	/*  int i=st.executeUpdate("delete from form10_registration where id="+id+"");    
	 int j=st3.executeUpdate("delete FROM form10_registration_item WHERE form10_no='"+FormNo+"'"); */ 
	 
	 int i=st.executeUpdate("UPDATE form10_registration SET deleted='yes' where id="+id+"");
...
/* 	 deleteBy.trim();
	...
	// postMail.postMail(deleteMail,deleteBy,"pass1234",mail_id," IT CLEARANCE FORM :  " +FormNo+ " ",deleteBy+  " "+  "Deleted IT CLEARANCE FORM :     "+FormNo+"");              
	 ...
	 response.sendRedirect("registerForm.jsp?msg="+msg);
	 }
	 catch (Exception e) {
