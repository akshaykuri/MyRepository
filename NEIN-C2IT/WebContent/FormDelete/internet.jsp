...
<title>INTERNET ACCESS REPORT</title>
...
{   
	...
	if(confirm('You are above to delete a Internet Access form'))
	{
var f=document.form;    
f.method="post";    
f.action='internetDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">INTERNET ACCESS FORM</td>
...
String query = "SELECT f.id,f.form6_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM  form6_internet f INNER JOIN   form6_internet_item g ON(f.form6_no=g.form6_no) WHERE Approval_status='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
