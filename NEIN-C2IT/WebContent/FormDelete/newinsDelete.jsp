...
<jsp:useBean id = "postMail" class="CON2.PostMail" /> 
 ...
 {          
	 ...
	 rs = st2.executeQuery("SELECT id,form7_no FROM  form7_newins WHERE id="+id+"");
	 if(rs.next()){
...
/* 	 rs2 = st4.executeQuery("SELECT id,emp,form1_no,mail FROM form1_it_clearance_item WHERE form1_no="+FormNo+" ");
	 if(rs2.next()){
		 ...
		 FormNo = rs2.getString("mail");
	        ...
	        /*         int i=st.executeUpdate("delete from   form7_newins where id="+id+"");    
	            	int j=st3.executeUpdate("delete FROM   form7_newins_item WHERE form7_no='"+FormNo+"'");
	            	int k=st3.executeUpdate("delete FROM   form7_allowed WHERE form7_no='"+FormNo+"'");
	            	int l=st3.executeUpdate("delete FROM   form7_blocked WHERE form7_no='"+FormNo+"'"); */
	            	
	           	 int i=st.executeUpdate("UPDATE form7_newins SET deleted='yes' where id="+id+"");
...
/* 	 deleteBy.trim();
	...
	// postMail.postMail(deleteMail,deleteBy,"pass1234",mail_id," IT CLEARANCE FORM :  " +FormNo+ " ",deleteBy+  " "+  "Deleted IT CLEARANCE FORM :     "+FormNo+"");              
	 ...
	 response.sendRedirect("newins.jsp?msg="+msg);
	 }
	 catch (Exception e) {
