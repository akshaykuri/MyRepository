...
<title>IT CLEARANCE REPORT</title>
...
{   
	...
	if(confirm('You are above to delete a RESIGNED EMPLOYEE IT CLEARANCE CHECK LIST'))
	{
var f=document.form;    
f.method="post";    
f.action='ItClearanceDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">RESIGNED EMPLOYEE IT CLEARANCE CHECK LIST</td>
...
//String query = "SELECT f.id,f.form1_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp SEPARATOR ', ') as emm FROM form1_it_clearance f INNER JOIN form1_it_clearance_item g ON(f.form1_no=g.form1_no) WHERE f.form1_no='NEIN/CS/IT/90-82' limit 1 ";
...
//String query = "SELECT f.id,f.form1_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM form1_it_clearance f INNER JOIN form1_it_clearance_item g ON(f.form1_no=g.form1_no) WHERE Approval_status='Pending' AND f.deleted IS NULL GROUP BY f.id ORDER BY f.id";
  String query = "SELECT f.id,f.form1_no,f.i_name,f.full_name,f.i_city,f.branch,f.f_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM form1_it_clearance f INNER JOIN form1_it_clearance_item g ON(f.form1_no=g.form1_no) WHERE f.Approval_sttatuss='Pending' AND f.deleted IS NULL GROUP BY f.id ORDER BY f.id";
