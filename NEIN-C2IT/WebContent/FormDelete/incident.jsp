...
{   
	...
	if(confirm('You are above to delete a Incident form'))
	{
var f=document.form;    
f.method="post";    
f.action='incidentDelete.jsp?id='+id;    
f.submit();
}
	else{}
}
...
if (VERSION > 3) {
	...
	<td colspan="8" align="center">INCIDENT FORM</td>
...
String query = "SELECT f.id,f.form9_no,f.SessionName,f.SessionCity,f.Created_date,GROUP_CONCAT(g.emp ORDER BY g.id ASC SEPARATOR ', ') as emm FROM  form9_incident_log f INNER JOIN  form9_incident_log_item g ON(f.form9_no=g.form9_no) WHERE Approval_sttatuss='Pending' AND f.deleted='no' GROUP BY f.id ORDER BY f.id";
...
<td bgcolor="white" width="100" colspan="2"><font size="2" color="black"><%=rs.getString(6)%></font></td>
