...
//int i=st.executeUpdate("update complain_master_item set cm_time2=CURRENT_TIMESTAMP(),cm_status='"+cm_status+"',cm_date2='"+result+"',admin_remarks='"+admin_remarks+"',status='append' where complain_id='"+complain_id+"'");                                                           

psmt = conn.prepareStatement("update complain_master_item set cm_time2=CURRENT_TIMESTAMP(),cm_status=?,cm_date2=?,admin_remarks=?,status=? WHERE complain_id=?");
psmt.setString(1,cm_status);
psmt.setDate(2,result);
psmt.setString(3,admin_remarks);
psmt.setString(4,"append");
psmt.setString(5,complain_id);

int i = psmt.executeUpdate();
          //out.println(" DETAIL   IS   INSERTED   SUCCESSFULLY");
           msg = "DETAIL__ \b SAVED__ \b\b SUCCESSFULLY";
           response.sendRedirect("complainPageStatus.jsp?msg=" +msg);
//}
%>
...
<td><input type="button" value="OK" onclick="window.location.href='complainPageStatus.jsp?msg=1'"/></td>
</tr>
