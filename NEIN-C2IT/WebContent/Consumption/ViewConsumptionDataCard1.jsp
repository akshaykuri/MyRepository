			...
			<tr class="blueth1">
             ...
             <%              
                           ...
                           try
                              {
   							...
   							String query = null;
   						        query = " SELECT DISTINCT `group_name` , `branchid`, `branchNo`, `number`, `relationnumber`, "+
   						                "`billfromdate`, `billtodate`, `monthofpayment`, "+
   						                "`invoiceno`, `invoiceamt`, `invoicedate`, `chequenumber`, `chequeamt`, `chequedate`, "+
   										...
   										" WHERE createddate  >=  ?"+
   										" AND createddate  <=  ?"+
   										" AND c.branchNo = ? AND  group_name !='null' group by group_name";
	              ...
	              %>
					 <tr class="bluetd1">
						<td ><%=++number%></td>
						<td ><%=rs1.getString("group_name")%></td>
						...
						<td bgcolor="white" width="150">
						     ...
						     <a href="EditConsumptionGroupDetails1.jsp?monthofpayment=<%=rs1.getString("monthofpayment")%>&fromdate=<%=fromdate1%>&todate=<%=todate1%>&group_name=<%=rs1.getString("group_name")%>&branch=<%=branch%>">Edit/Update</a>
						     </font>
						</td>   							  
						</tr>
						 <%}%>
