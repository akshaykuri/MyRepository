       ...
       if (document.getElementsByName("mmdradio")[0].checked && document.getElementsByName("higherAuthority")[1].checked )
    	   ...
    	   var nui =document.form.ITusername;
	 	   for(var j=0;j<nui.length;j++)
	 	      ...
	 	      if(j==nui.length)
	 	    	  {
	 	    	  alert("Select Internet Access Provided");
	 	    	 nui[0].focus();
	 	    	  return false;
	 	    	  }
 ...
 */
  ...
  <script language="javascript" type="text/javascript"> 
  function resendMail(mail,form6_no,email_id,sessionName)
  		 {
	if (typeof XMLHttpRequest != "undefined"){    
         xmlHttp= new XMLHttpRequest();    
           }      else if (window.ActiveXObject){
           alet("Creating html http object");    
             xmlHttp= new ActiveXObject("Microsoft.XMLHTTP");  
                    ...
                    alert("Browser does not support XMLHTTP Request");    
                      return;     
                       }       var url="../../../ResendMailInternetAccess?mail="+mail+","+form6_no+","+email_id+","+sessionName+"";  
...
<form  name="form" method="post"  action="InternetAccessAdminBack.jsp">
...
rs100 = st100.executeQuery("select * from form6_internet WHERE form6_no='"+req+"'"); 
while(rs100.next())
{
...
<table width="101%"  cellspacing="0" cellpadding="0" bgcolor="grey" align="center">
	<tr>
		...
		<input type="hidden" name="form6_no" value="<%=form6_no%>" />
...
<td style="text-align:left;" bgcolor="grey"><input name="branch" value="<%=form6_no%>" size="31" readonly="readonly"/> </td>
</tr>
<tr>
...
<table id="tableData" name="tableData" width="100%" border="1"  align="center" bgcolor="#F0F0F0" bordercolor="grey">
	  ...
	  rs1 = st1.executeQuery("select form6_no,desg,emp,status,remarks,f_date,mail,empid,levelno from form6_internet_item WHERE form6_no='"+req+"' ORDER BY id ASC"); 
	  while(rs1.next())
	  {
		  String form6_noD= rs1.getString("form6_no");
	 ...
	 <td><input type="button"  style="size:1" value="RE-SEND MAIL" onclick="resendMail('<%=m%>','<%=form6_no%>','<%=email_id%>','<%=SessionName%>');" /></td> 
...
String flag =c2ITUtil1.getAuthorizedNameInternet(form6_no,session.getAttribute("Nname").toString(),session.getAttribute("desg").toString(),session.getAttribute("EmpNo").toString()); %>
   ...
   <tr style="display: none;" id="idApproved" >
    ...
    cn5 = DriverManager.getConnection(url+dbName,userName,password); 
     ...
     String maxlevel="";
	  ...
	  String i_b_no1=form6_no.substring(17,19);
	  
	  
	  
      rs12= st12.executeQuery("SELECT max(levelno) FROM formapproval WHERE branch='"+i_b_no1+"' and formtype='INTERNET_ACCESS'  ORDER BY levelno ASC");
     if(rs12.next())
         { 
    ...
    rs11 = st11.executeQuery("SELECT alloptions,directApprove FROM formapproval WHERE branch='"+i_b_no1+"' and empid='"+emp_no+"' and formtype='INTERNET_ACCESS'");
     if(rs11.next())
         { 
     ...
     /* ////////////////////////// Skeeping level code  ////////////////////////////////// */
      ...
      rsnext = stnext.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='INTERNET_ACCESS' ORDER BY levelno ASC");
      if(rsnext.next())
           {
     ...
     if(allopt.equals("0") || allopt.equals(""))
    	  ...
    	  if(!levelno.equals(maxlevel) && !dirappro.equals("1"))
    	     ...
    	     rs5 = st5.executeQuery("SELECT * FROM formapproval WHERE branch='"+i_b_no1+"' and levelno='"+nextlevel+"' and formtype='INTERNET_ACCESS' ORDER BY levelno ASC");
    	     if(rs5.next())
                {
               ...
               %><input type="hidden" name="senddirect" id="senddirect" value="sendonly" readonly/>
               ...
               else
                  {
            	   ...
            	   napprovalsw1="2645"+","+developer_name+","+"Manager"+","+developer_mail+","+"Creater";
            	   ...
            	   if(!levelno.equals(maxlevel) && dirappro.equals("1"))
            	         ...
            	         <td style="display: none;" id="send">Send to HO(IT) for Create<br>
                           ...
                           <td style="display: none;" id="direct">Create Internet Access<br>
            	           <table id="idFinal" align="center" width="100%" border="1" style="height: 150;" cellspacing="0" cellpadding="0" bgcolor="lightgrey" >
                             <tr ><td class="it2" style="text-align: right;width:23%;">Internet Access Provided:</td>
	<td class="it2" style="text-align: right;width:23%;">
	
	<input type="radio" name="ITusername" value="Privilege Access"/>Privilege Access
	<input type="radio" name="ITusername" value="Limited Access"/>Limited Access
	<input type="radio" name="ITusername" value="Cancelled Internet Access"/>Cancelled Internet Access
	</td></tr>
<!-- <tr>
