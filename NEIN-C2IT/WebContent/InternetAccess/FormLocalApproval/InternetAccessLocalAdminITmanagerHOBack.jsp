...
psmt = cn2.prepareStatement("UPDATE form6_internet_item f,(SELECT max(id) as idf FROM form6_internet_item WHERE form6_no=? AND desg=? AND emp=?) as p SET f.status=?,f.remarks=?,f.f_date=?,f.presence=? WHERE f.form6_no=? AND f.desg=? AND f.emp=? AND f.id=p.idf");
...
psmt2 = cn2.prepareStatement("UPDATE form6_internet SET ITusername=?,ITpassword=?,ITcreated=?,ITremark=?,Approval_sttatuss=?,Approval_f_date=? WHERE form6_no=?");
...
{
postMail.postMail(i_email_id,i_name,"pass1234",mailingCheck[u]," INTERNET ACCESS FORM: :  " +form_no+ " ",i_name+  " "+  "request is Approved " +form_no+ " <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://ninsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
}
...
response.sendRedirect("InternetAccessSelectLocalAdmin.jsp?msg=" +msg);
