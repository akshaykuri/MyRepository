...
if((mmdradio!=null && mmdradio.equals("Approved"))) 
    { System.out.println("approved :"+empid1);
	if(!empid1.equals("Final") && !empid1.equals("Final1"))
	    { 
		System.out.println("approved1 :"+empid1);
		psmt = cn2.prepareStatement("UPDATE  form10_registration_item SET status=?,remarks=?,presence=? WHERE form10_no=?  and empid='"+session.getAttribute("EmpNo")+"' ");
	    ...
	    psmt10 = cn2.prepareStatement("insert into form10_registration_item (form10_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) values (?,?,?,?,?,?,?,?,?,?)");
	    ...
	    psmt2 = cn2.prepareStatement("UPDATE form10_registration SET ITusername=?,ITpassword=? WHERE form10_no=?");
		...
		psmt4 = cn2.prepareStatement("SELECT mail,emp FROM form10_registration_item WHERE form10_no=?");
		psmt4.setString(1,form_no);
        ResultSet rs1 = psmt4.executeQuery();
		while(rs1.next())
		    {
		    allMails1 = rs1.getString(1); 
			postMail.postMail(i_email_id,i_name,"pass1234",allMails1,"NEIN-C2IT INTERNAL NEIN SOFTWARE USER - REGISTRATION FORM :  " +form_no+ " ",i_name+  " "+  "Created User has per the " +form_no+ " request <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
	        }
		String ccmailf="nilkanth.pawar@nipponexpress.com";
		postMail.postMail(i_email_id,i_name,"pass1234",ccmailf,"NEIN-C2IT INTERNAL NEIN SOFTWARE USER - REGISTRATION FORM :  " +form_no+ " ",i_name+  " "+  "Created User has per the " +form_no+ " request <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);

	    }
     else
        { System.out.println("approved 2:"+empid1);
 	    psmt = cn2.prepareStatement("UPDATE  form10_registration_item SET status=?,remarks=?,presence=? WHERE form10_no=?  and empid='"+session.getAttribute("EmpNo")+"' ");
 	    psmt.setString(1,mmdradio);
 	    psmt.setString(2,txtmail_msg);
 	    psmt.setString(3,"over");
       ...
       psmt11 = cn2.prepareStatement("insert into form10_registration_item (form10_no,desg,emp,status,remarks,f_date,mail,presence,empid,levelno) values (?,?,?,?,?,?,?,?,?,?)");
       ...
       int b = psmt11.executeUpdate(); 
        ...
        message = mailMessage.getMailMessageRegistration(form_no.trim());
 	    postMail.postMail(i_email_id,i_name,"pass1234",mail,"NEIN-C2IT NEIN USER - REGISTRATION FORM :  " +form_no+ " ","Dear "+emp+","+"<br>Request you to Create User Credential in NEIN Application of Form No :" +form_no+ "  <br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
        }
    }
else
    {
	...
	int i = psmt.executeUpdate();
	message = mailMessage.getMailMessageRegistration(form_no.trim());
	postMail.postMail(i_email_id,i_name,"pass1234",mail,"NEIN-C2IT NEIN USER - REGISTRATION FORM :  " +form_no+ " ", "Your request for NEIN user registration form no :" +form_no+ " not approved by :" +i_name+ "<br><br><b><u> Open the following URL</u> </b><br> C2IT LINK -  https://neinsoft.nittsu.co.in:8185/NEIN-C2IT/HOME.jsp  <br> COMMON LINK - https://neinsoft.nittsu.co.in:8185/NEIN  <br><br><br><br>"+message);
    }

message = null;
msg = "NEIN__USER__REGISTRATION__SAVED__AND__SENT__SUCCESSFULLY";
