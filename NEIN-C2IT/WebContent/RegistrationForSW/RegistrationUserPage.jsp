...
<table width="907"  cellspacing="0" cellpadding="0" bgcolor="grey" align="center">
	<tr>
		...
		<b><label>
		 NEIN APPLICATION USER - REGISTRATION FORM</label>
		</b></font></center> 
		</td>
		</tr>
...
rs3 = st3.executeQuery("SELECT max(idextend) FROM form10_registration WHERE form10_no IN(SELECT form10_no FROM form10_registration LEFT JOIN branch_master b ON(substr(form10_no,22,2)=b.b_no) WHERE '"+b_no+"'=b.b_no GROUP BY form10_no)");
...
{   */  
	...
	catch(SQLException ex)
	{
		 ...
		 new_id = "NEIN/CS/Registration/90-1";
	    }
...
<tr>
<td  style="text-align: right;"><font color=red>*</font>Software  :</td>

<%-- <td style="text-align:left;"><select name="Software"  id="hiderow" style="font-size : 8pt">
...
<td >
	                        <div class="multiselect">
                                   ...
                                   <div id="checkboxes"  onclick="showCheckboxes()">
                                        <%while(rs5.next()){%> 
     	                                     <label for="<%=rs5.getString("app_name")%>" style="width:55%; text-align: left;">
      			                             <input type="checkbox" name="approverslist" onclick="sync(this),showCheckboxes()" id="<%=rs5.getString("app_name")%>" class="group1" value="<%=rs5.getString("app_name")%>" /><%=rs5.getString("app_name")%></label>
     	      	                        <%}%>
                                  </div>
                             </div>
