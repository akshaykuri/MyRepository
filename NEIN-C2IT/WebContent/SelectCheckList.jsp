...
rs9 = st9.executeQuery("SELECT DISTINCT c.cs_id FROM cs_master c INNER JOIN branch_master b ON((substr(c.cs_id,9,2))='"+city_no+"') WHERE c.cs_date='"+dateF+"'GROUP BY c.cs_id ");
/* while(!rs9.next())
{
...
%>
<form name="form1" method="post" action="CheckListViewTextFormBranch.jsp?msg=1" onsubmit="return validate();">
  ...
  <table width="280" align="center" border="0" cellpadding="3" cellspacing="1" bgcolor="#ec691f">
   ...
   <tr><!--<th>BRANCH:</th>
	--><td>
	  ...
	  if (rs9 != null) { try { rs9.close(); } catch (SQLException e) { /* ignored */}}
	  if (st9 != null) { try { st9.close(); } catch (SQLException e) { /* ignored */}}
	  if (cn9 != null) { try { cn9.close(); } catch (SQLException e) { /* ignored */}}
      }
										        %> 
										        </select>
      </td>
...
<a href="CheckSheetSelectionBranch.jsp?msg=1"><img src="bakk.png" border="0" height="35" width="35"></a><address><font size="2">BACK</font></address> 
</td> 
</tr> 
</table> 
