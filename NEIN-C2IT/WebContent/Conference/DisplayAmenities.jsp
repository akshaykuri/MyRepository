...
function editRecord(id)
    ...
    var f=document.form;    
    f.method="post";    
    f.action='amenitiesEdit.jsp?id='+id;    
    f.submit();
    }
	...
	if(confirm('You are above to delete Amenities'))
	      {
           var f=document.form;    
           f.method="post";    
           f.action='amenitiesDelete.jsp?id='+id+'&id1='+id1;    
           f.submit();
          }
	else{}
   }
...
<table width="1118" align="center" border="0" cellspacing="0" cellpadding="0">
 ...
 <tr align="center">
 <td bgcolor="#2f4377" colspan="2"><strong><font size ="4" color="white">AMENITIES  DETAILS</font></strong></td></tr>
...
<table border="1"  align="center" bgcolor="#F0F0F0" bordercolor="#74c1b9">
	<tr>
	  ...
	  <td bgcolor="#74c1b9" width="120" align=center><font color="black">Quantity</font></td>
		<td bgcolor="#74c1b9" width="120" align=center><font color="black">Remarks</font></td>
 ...
 try 
  ...
  //String query = "select assettype,configname from assetconfgfield where assettype='"+an+"' and configname ='"+co+"'";
   ...
   //rs = st.executeQuery(query);
    ...
    String query1 = "select * from amenities_add where Booking_no='"+no+"'";
   st2 = conn.createStatement();
   rs2 = st2.executeQuery(query1);  %>
   <%while(rs2.next()){%>
    <tr>
         <%--  <td style="visibility: hidden;"><input type="hidden" name="sid" value="<%=sid%>"></td> --%>
         <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("items")%>" readonly></td>
         <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("quantity")%>" readonly></td>
   	     <td bgcolor="#d2f7f3"><input name="user" style="background-color:white;width:100;color:black;" value="<%=rs2.getString("remarks")%>" readonly></td>
         <td bgcolor="#d2f7f3"><input id="hiderow" type="button" size="2" name="edit" value="Edit" style="background-color:white;font-weight:bold;color:black;" onclick='editRecord("<%=rs2.getInt("aid")%>");' ></td>
         <td style="background-color:#d2f7f3"><input id="hiderow" type="button" size="2" name="delete" value="Delete" style="background-color:white;font-weight:bold;color:black;" onclick='deleteRecord("<%=rs2.getInt("aid")%>","<%=rs2.getString("Booking_no")%>");' ></td>
 </tr>
	  <%}%>
